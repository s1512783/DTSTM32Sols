...
{
	HAL_SPI_Transmit(&hspi2, buff, bc, HAL_MAX_DELAY);
}
...
{
	HAL_SPI_Receive(&hspi2, buff, bc, HAL_MAX_DELAY);
}
...
{
	BYTE n, ty, cmd, buf[10];
	UINT tmr;
	DSTATUS s;

	BYTE dummybuf [10] = {0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF};

	if (drv) return RES_NOTRDY;

	deselect();				/* Initialize control port */
	for (n = 10; n; n--) rcvr_mmc(buf, 1);	/* 80 dummy clocks */

	HAL_SPI_TransmitReceive(&hspi2, dummybuf, buf, 10, HAL_MAX_DELAY);
...
DWORD get_fattime (void){
	return ((DWORD)(2018 - 1980) << 25 | (DWORD)5 << 21 | (DWORD)13 << 16);
