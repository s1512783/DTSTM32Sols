		...
		unsigned char c, uint16_t textColor, uint16_t bgColor)
{
	...
	//ST7735_setAddrWindow(x0, y0, x0 + 9, y0 + 6, MADCTLTEXT);
		...
		if (!endString){
			for (j = 0; j < lineLen; j++) {
				if (*str == '\n'){ // skip line if newline
					for(; j < lineLen; j++)
						ST7735_drawChar(x0 + j*CHARWIDTH, y0 + i*CHARHEIGHT, font[0], textColor, bgColor); // print empty chars till end of string
					str++;
					break;
				}
